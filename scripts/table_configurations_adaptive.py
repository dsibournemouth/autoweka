...
def table_header(complete):
    ...
    if complete:
        table += '<thead><tr><th>dataset</th><th>strategy</th><th>generation</th><th>seed</th><th>batch</th> \
              <th>missing values</th><th>outliers</th><th>transformation</th><th>dimensionality reduction</th> \
              <th>sampling</th><th>predictor</th><th>meta</th><th>CV error</th><th>Test error</th><th>Evaluations</th> \
              </tr></thead>\n'
    else:
...
def table_row(tr_class, dataset, strategy, generation, seed, params, error, test_error, num_evaluations, batch,
              complete):
    signal_plot = '../plots%s/signal.%s.%s.%s.%s.png' % (suffix, dataset, strategy, generation, seed)
    if complete:
        return '<tr style="%s"><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td>' \
               ...
               '<td>%s</td><td><a href="%s">%s</a></td><td>%s</td></tr>\n' % (
                   tr_class, dataset, strategy, generation, seed, batch,
...
def get_results(dataset, strategy, generation):
    ...
    c.execute('''SELECT dataset, strategy, generation, seed, configuration, error, test_error, num_evaluations, batch
                 FROM results WHERE dataset='%s' AND strategy='%s' AND generation='%s' ORDER BY seed
    ...
    # TO FIX -- batch is not taken into account here
