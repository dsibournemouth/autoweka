...
FOLDS=10
#echo $1 $2 $3
$MY_JAVA_PATH/java -Xmx18000m -cp $AUTOWEKA_PATH/autoweka.jar $METHOD -t $AUTOWEKA_PATH/datasets/$DATASET/train.arff -x $FOLDS -s $SEED -o | grep "Correctly Classified Instances" | sed 1d | awk '{ print $5 }' > $AUTOWEKA_PATH/experiments/defaultParameters/$DATASET.$METHOD.CV.$SEED.csv
$MY_JAVA_PATH/java -Xmx18000m -cp $AUTOWEKA_PATH/autoweka.jar $METHOD -t $AUTOWEKA_PATH/datasets/$DATASET/train.arff -T $AUTOWEKA_PATH/datasets/$DATASET/test.arff -s $SEED -o | grep "Correctly Classified Instances" | sed 1d | awk '{ print $5 }' > $AUTOWEKA_PATH/experiments/defaultParameters/$DATASET.$METHOD.Test.$SEED.csv
