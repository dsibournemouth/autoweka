...
def scatter_all_seeds(results, title, seeds):
    plt.close()
    results = np.array(results)

    print results.shape
    adaptive = results.shape[1] > 3
    ...
    if adaptive:
        for batch in range(1, 9):
            for seed in seeds:
                ...
                mask = mask & (results[:, 3] == int(batch))
                time = (TIME_LIMIT*batch + results[mask, 1]) / 60 / 60
                error = results[mask, 2]
                if is_regression:
                    error = np.log10(error)

                try:
                    plt.plot(time, error, linestyle='solid', linewidth=.5, marker='o', markersize=3,
                             color=cm.hsv(float(seed) / number_seeds, 1), alpha=.75)
                except:
                    continue
    else:
        for seed in seeds:
            ...
            plt.plot(time, error, linestyle='solid', linewidth=.5, marker='o', markersize=5,
                     color=cm.hsv(float(seed) / number_seeds, 1), alpha=.75)
...
def aggregated_line_seeds(results, title, seeds):
    plt.close()
    ...
    for dataset in selected_datasets:
        for strategy in selected_strategies:
            for generation in selected_generations:
                ...
                if args.adaptive:
                    query = "SELECT seed, time, error, batch FROM trajectories WHERE dataset='%s' AND strategy='%s' " \
                            "AND (generation='%s' or generation='%s-%s') AND error<1000000000 " \
                            "AND time<=%d" % (dataset, strategy, generation, generation, dataset, TIME_LIMIT)
                else:
                ...
                scatter_all_seeds(results, title, selected_seeds)
                aggregated_line_seeds(results, title, selected_seeds)
