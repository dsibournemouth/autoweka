...
public abstract class ExperimentConstructor {
   ...
   */
  protected String mParamBaseDir = autoweka.Util.getAutoWekaDistributionPath()
      + File.separator + "params";
  ...
  /**
   * The instance generator we're using, all loaded up and ready to go with the
   * appropriate dataset
   */
  ...
  public static void main(String[] args) {
    // Is the first argument a -batch? If it is, then we need to load the given
    // xml files and use those to generate things
  ...
  public static void generateBatches(String xmlFile) {
	...
	for (ExperimentBatch.DatasetComponent datasetComp: batch.mDatasets) {
	  ...
	  buildSingle(expComp.constructor, exp, new LinkedList<String>(
	      expComp.constructorArgs));
	}
      }
    }
    catch (Exception e) {
  ...
  public static void buildSingle(String builderClassName, Experiment exp,
    ...
    try {
      ...
      throw new RuntimeException("Could not find class '" + builderClassName
	  + "': " + e.getMessage(), e);
    }
    catch (Exception e) {
      throw new RuntimeException("Failed to instantiate '" + builderClassName
	  + "': " + e.getMessage(), e);
    }
  ...
  private void run(Experiment exp, List<String> args) {
    // See if we can load up this constructor's canonical class name's props
    // file
    mProperties = new Properties();
    String propsFilePath = Util.getAutoWekaDistributionPath() + File.separator
    ...
    // Generate all the stuff that needs to be created alongside the experiment
    // file
  ...
  protected String getWrapperPropString() {
     ...
     * 
     * sb.append(":regularizerParams="); if(mExperiment.regularizerArgs != null)
  ...
  /**
   * Get a string indicating the type of this Experiment (namely the name of the
   * SMBO method)
   */
  ...
  private void checkPrefixes() {
	...
	if (prefixes.contains(prefix)) {
	  throw new RuntimeException("Prefix '" + prefix + "' ("
	      + param.getTargetClass() + ") is already in used by somebody");
	}
	prefixes.add(prefix);
      }
    }
  ...
  /**
   * Gets a prefix out of the classifier name by stripping all the packages and
   * capital letters - needed to ensure that parameters with the same WEKA name
   * don't collide.
   */
  ...
  /**
   * Populates a ParameterConditionalGroup with all the params/conditionals that
   * are needed for optimization methods that support a DAG structure
   */
  public ParameterConditionalGroup generateAlgorithmParameterConditionalGroupForDAG() {
	...
	throw new RuntimeException("Couldn't find any attribute search methods");
      ...
      // Insert NONE for now, we need to take it out in a second.... after we
      // build the param
    ...
    // First, insert the top level choice about which classifer is going to be
    // selected
    ...
    // Next, insert all the default parameters for each method (Just the flat
    // level
    if (mIncludeBase) {
      for (ClassParams clsParams: mBaseClassParams) {
	addClassifierToParameterConditionalGroupForDAG(paramGroup, clsParams,
	...
	Parameter gateParam = new Parameter(prefix + "_0_QUOTE_START_B",
	    baseClassifiers);
	paramGroup.add(gateParam);
	for (ClassParams clsParams: mBaseClassParams) {
	  addClassifierToParameterConditionalGroupForDAG(paramGroup, clsParams,
	      prefix + "_1_", gateParam);
	}
	...
	paramGroup.add(new Conditional(gateParam, _HIDDEN_ensemble_depth,
	    levels));
	// Make the dummy param to close the quote
	Parameter endQuote = new Parameter("_1_" + String.format("%02d", i)
	    + "_2_QUOTE_END", "REMOVED");
	paramGroup.add(endQuote);
	paramGroup
	    .add(new Conditional(endQuote, _HIDDEN_ensemble_depth, levels));
      }
    }
  ...
  /**
   * Internal helper method that adds a child classifier's params assuming a DAG
   * structure
   */
  private void addClassifierToParameterConditionalGroupForDAG(
      ParameterConditionalGroup paramGroup, ClassParams clsParams,
